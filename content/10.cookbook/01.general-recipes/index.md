---
title: 一般的なレシピ
lastmod: 2026-03-25T16:44:29+09:00
description: 'Grav のコア機能や一般的な利用における、便利なテクニックをまとめて紹介します。'
weight: 10
params:
    srcPath: /cookbook/general-recipes
---

このページでは、一般的な Grav に関するさまざまな問題と、それぞれの解決策を紹介します。

## PHP CLI のバージョンを変更{#change-the-php-cli-version}

ときどき、web サーバーが実行する PHP のバージョンと、ターミナルで実行される PHP のバージョンが異なる場合があります。

CLI で動いている PHP のバージョンは、 `php -v` コマンドの実行で確認できます。
Grav のシステム要件は少なくとも PHP 5.5.9 以上であるため、 PHP バージョンが 5.5.9 未満の場合、起動しないでしょう。

> [!訳注]  
> 上記の PHP バージョンは古い情報で、Grav 1.7 は PHP 7.3.6 以上、Grav 1.8 は、PHP 8.3 以上になると思われます。
...
## シンプルなギャラリーを作る{#creating-a-simple-gallery}
...
## カラムに分けてコンテンツをレンダリングする{#render-content-in-columns}
...
## 本当にシンプルな CSS 画像スライダー{#really-simple-css-image-slider}
...
## マークダウンを HTML に入れる{#wrapping-markdown-into-html}
...
## サイドバーに最近の投稿ウィジェットを追加する{#add-a-recent-post-widget-to-your-sidebar}
...
## プライベートエリアを作る{#create-a-private-area}

Grav なら、 web サイトにプライベートエリアを作るのも簡単です。
すべて Login プラグインのおかげです。

## サイトの一部にアクセスするためにユーザーにログインを要求する{#require-users-to-login-prior-to-access-a-part-of-the-site}
...
## ページの閲覧に特別なパーミッションを要求する{#require-special-permissions-to-view-one-or-more-pages}
...
## グループ単位のパーミッションを使う{#use-group-based-permissions}
...
## フッターにJavaScript を追加する{#add-javascript-to-the-footer}
...
## デフォルトのログフォルダの場所を上書きする{#override-the-default-logs-folder-location}
...
## メニューシステムを縦に分ける{#split-vertical-menu-system}
...
ここでは、 Grav 内のすべての公開ページを繰り返して、順番付きリストを作成しています。
...
このコードは、常に Twig 構造の **後に** 置かなければいけません。
こちらも、理想的には [アセット管理](../../03.themes/07.asset-manager/) を使ってください。

## 1つ以上のページで動的にスタイルをあてる{#dynamically-style-one-or-more-pages}
...
## HTML テーマを Grav に移植する{#migrate-an-html-theme-to-grav}
...
#### アセットリンクを正しく追加する{#adding-the-correct-asset-links}

Grav では、ホームへのルーティングを指しており、 Grav のルートフォルダで、 `/user/themes/mytheme/img` ではなく、 `/img` を指しているため、 link は壊れます。
...
#### 他のページも追加する{#adding-another-page}
...
## 特定のページにだけアセットを追加する{#add-an-asset-to-a-specific-page}
...
## ページやモジュラーコンテンツを他のページで再利用する{#reuse-page-or-modular-content-on-another-page}
...
## 問い合わせフォームにカスタムのアンチ・スパムフィールドを作る{#make-a-custom-anti-spam-field-for-your-contact-for}
...
## 異なる環境で異なる robots.txt コンテンツを表示する{#display-different-robots-txt-contents-for-differen}
