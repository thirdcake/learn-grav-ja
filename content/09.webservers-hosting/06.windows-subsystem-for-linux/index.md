...
weight: 60
params:
    srcPath: /webservers-hosting/windows-subsystem-for-linux
---
...
## WSL のインストール{#installing-windows-subsystem-for-linux}
...
## Apache のインストール{#installing-apache}
...
## Grav をインストール{#installing-grav}

Grav は Windows 内からでも Ubuntu 内からもインストールできます。

#### オプション1： Windows{#option-1-windows}
...
#### Option 2: Ubuntu{#option-2-ubuntu}
...
## XDebug のインストール（オプション）{#installing-xdebug-optional}
...
#### Visual Studio Code でデバッガを使う（オプション）{#launching-debugger-in-visual-studio-code-optional}
...
## 仮想ホストの追加（オプション）{#adding-extra-virtual-hosts-optional}
...
## Apache を自動で起動する（オプション）{#automatically-start-apache-optional}
...
### GUI Linux ターミナルエミュレータ{#gui-linux-terminal-emulator}
...
### 1つの Grav コードベースで複数のweb サイト{#multiple-websites-one-grav-codebase}
