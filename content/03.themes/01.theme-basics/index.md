...
## ページコンテンツと Twig テンプレート{#content-pages-twig-templates}
...
## テーマの構成{#theme-organization}

### 定義と設定{#definition-configuration}
...
### テーマとプラグインのイベント{#theme-and-plugin-events}
...
上記のとおり、イベントフックを利用するためには、まず `getSubscribedEvents` 関数にそれらを登録し、それからコードを定義する必要があります。  
イベントを登録したら、そのコードも定義する必要があります。そうしなければ、エラーになります。

### 他のフォルダ{#other-folders}
