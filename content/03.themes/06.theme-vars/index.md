...
### `language_codes` 変数{#language-codes-variable}

サイトで利用できる言語コードのリストを返します。

### `assets` オブジェクト{#assets-object}
...
## カスタム変数を追加{#adding-custom-variables}
...
一方、特定のページでのみ必要な変数であれば、ページの YAML フロントマターで変数を追加できます。  
...
## カスタムオブジェクトを追加{#adding-custom-objects}
