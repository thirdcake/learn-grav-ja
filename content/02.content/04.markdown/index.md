---
title: マークダウン構文
lastmod: '2026-07-27T21:25:43+09:00'
...
### iframe と 生の HTML の許可{#allowing-iframes-and-other-raw-html}
...
- **tagfilter 拒否リストが固定** （`title`, `textarea`, `style`, `xmp`, `iframe`, `noembed`, `noframes`, `script`, `plaintext`） され、そのリストが編集不可。これは、 GitHub フレーバーマークダウンの仕様に準拠します。

embed に戻すのに、良い方法が2つあります：

1. **推奨： ショートコードやページテンプレートを介して追加し** マークダウンに生の HTML を渡さない。この方法により、他のすべての tagfilter はそのままにして、意図通り embed を置くことができます。 [ショートコードの作成](../06.creating-a-shortcode/) と [ページテンプレートアプローチ](../05.twig-in-content/#use-a-page-template) をご覧ください。
2. **tagfilter をオフにする** もしサイト全体で生の HTML を許して良い場合。設定は、 *設定>システム>マークダウン* もしくは、 `user/config/system.yaml` です：
