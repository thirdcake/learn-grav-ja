---
title: 'ページ'
lastmod: '2026-04-23T18:14:51+09:00'
description: 'ページは、Grav における最も重要な構成要素です。ページの作成方法や利用方法を解説します。'
weight: 10
params:
    srcPath: '/content/content-pages'
---
...
| プロパティ | 説明 |
| -------- | ------ |
| **default**  | ファイルシステムの順 （例） `01.home` の次に `02.advark` |
| **title**    | それぞれのページで定義された title の順 |
| **basename** | 数字順ではなく、そのフォルダのアルファベット順 |
...
つまり、多くのファイルのファイル名は、 `default.md` になります。
...
2つの `---` のマーカーの間にある設定は、 YAML フロントマターとして知られており、一般的な YAML の設定で構成されています。

この例では、タイトルと **blog** タクソノミーの設定がされています。  
2つ目の `---` の後にあるコンテンツが、サイトに HTML として表示される実際のコンテンツです。  
...
### 概要の文字数と区切り文字{#summary-size-and-separator}
...
> [!訳注]  
> 日本語の場合、英語と違い要約で300文字だと長過ぎるので、何かしらの調整をすることになると思います。[config 設定 &gt; site 設定](../../01.basics/05.grav-configuration/#site-configuration) にある、 summary からできます。
