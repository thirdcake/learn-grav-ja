---
title: 'Grav の開発'
lastmod: 2026-03-21T20:44:40+09:00
description: 'Grav による開発手法について解説します。コア、プラグイン、スケルトン、テーマの他、 GitHub のセットアップなども網羅しています。'
weight: 90
params:
    srcPath: /advanced/grav-development
---
...
#### スケルトンの要件{#skeleton-requirements}
...
## テーマ・プラグインのリリース手順{#theme-plugin-release-process}
...
> [!Tip]  
> バージョンは、 `vX.Y.Z` もしくは `X.Y.Z` のいずれでも可能ですが、リリース間で一貫してください。 `#` のバージョン、 `##` の（アメリカ式フォーマットの）日付ののち、改行があることを確認してください。また、箇条書きの項目は同じ数だけインデントしていることも確認してください。
...
## 放棄されたリソースのプロトコル{#abandoned-resource-protocol}
