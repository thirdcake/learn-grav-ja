...
weight: 50
params:
    srcPath: '/basics/grav-configuration'
---
...
これらの config オプションは、トップレベルで設定するものです。  
これらはサイトの運用、タイムゾーン、ベース URL に影響する一般的なオプションです。
...
| **default_lang:** | デフォルトでは、上記の supported に最初に書いた言語になります。サポート対象言語のリスト中から選ばなければいけません |
...
## その他の設定とファイル設定{#other-configuration-settings-and-files}
...
### 設定変数の名前空間{#config-variable-namespacing}
...
### プラグインの設定{#plugins-configuration}
