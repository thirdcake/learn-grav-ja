---
title: 'システム要件'
lastmod: 2026-03-18T17:43:24+09:00
description: 'Grav は、PHP といくつかの拡張だけで簡単に実行できます。軽量で幅広い環境に対応します。'
weight: 20
params:
    srcPath: '/basics/requirements'
---
...
Grav は、とてもシンプルで汎用的なので、動かすだけなら web サーバーさえ不要です。直接、 PHP 8.3.11 以上のビルトイン・サーバで動かせます。

ただし、ビルトイン・サーバによる動作テストは、 Grav のインストール確認や、簡単な開発作業には便利ですが、本番環境のサイトや、高度な開発作業には **推奨されません** 。  
...
* [AMPPS](https://www.ampps.com/downloads) Softaculous による Apache, PHP, Perl, Python, などのソフトウェアスタックです。Grav 開発に必要なもの（とそれ以上）がすべて揃っています。
...
* Linux の多くのディストリビューションには、すでに Apache と PHP が組み込まれています。そうでない場合も、そのディストリビューションで通常、パッケージマネージャを提供しており、それを使ってそれほど大変でなくインストールすることができます。より高度な設定は、優れた検索エンジンの助けを借りて調べてください。

### Apache の要件{#apache-requirements}
...
### IIS の要件{#iis-requirements}

IIS は、 'すぐ使える' web サーバーだと思われていますが、いくつかの設定が必要です。
...
##### 追加するとより良いモジュール{#optional-modules}

* `apcu` キャッシュパフォーマンスを向上させるため
* `opcache` PHP パフォーマンスを向上させるため
* `yaml` PECL Yaml は、ネイティブな yaml 処理を提供し、パフォーマンスが大幅に向上します
* `xdebug` 開発環境でのデバッグに便利です

### パーミッション{#permissions}

Grav が正しく機能するために、ログやキャッシュを書き込むための適切な **ファイル・パーミッション** が、 web サーバーには必要です。  
...
4. [MarkdownPad](https://markdownpad.com/) - Windows - 無料と、 Pro のバージョンあり。YAML フロントマターサポートまでも付いています。Windows ユーザーにとって素晴らしいソリューションです。
5. [Mark Text](https://github.com/marktext/marktext) - 無料で、オープンソースの Markdown エディタです。 Windows / Linux / MacOS 対応。

### FTP クライアント{#ftp-clients}
