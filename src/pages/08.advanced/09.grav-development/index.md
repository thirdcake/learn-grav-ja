...
3. Contains a `blueprints.yaml` file with [all required fields](../../06.forms/01.blueprints/). [Example Here](https://github.com/getgrav/grav-theme-antimatter/blob/develop/blueprints.yaml)
...
! Dates can use either the **American** `m/d/y` [date format](../../02.content/02.headers/#date), or the **European** `d-m-y` format. Also make sure there is an empty newline between the headers (version and date) and lists (new, improved, bugfix).
