...
Gravで言うところの **Page** とは、サイトを根本的に構成するブロックです。Gravシステムでは、これらページによって、コンテンツを書いたり、ナビゲーションを提供したりします。
...
**Gravコア** パッケージをダウンロードし、インストールしたとき、標準的なページに出迎えられます。シンプルな通常Pageの作り方は、[基本のチュートリアル](/01.basics/basic-tutorial/)で解説しています。

#### Listing Page

![リストページ](content-listing.png)
...
サイトには、エントリーポイントが必要です。エントリーポイントとは、ブラウザがサイトのルートにアクセスしたときに表示される場所です。たとえば、ブラウザに `http://yoursite.com` と打ち込んだとき、Gravは、デフォルトでは `home/` を表示します。しかし、[Grav設定ファイル](/01.basics/grav-configuration/) にある `home.alias` オプションを変更することで、ホームの場所を上書きすることができます。
...
> [!Tips]
> この設定は、そのページの自身のヘッダに、[表示パラメータ](/02.content/headers/#visible) を設定することで上書きできます。
...
> [!Tips]
> [Gravのシステム設定ファイル](/01.basics/grav-configuration/) の `pages.order.dir` と `pages.order.by` オプションを設定することで、**デフォルトの振る舞い** を上書きできます。
...
> [!Tips]
> この振る舞いは、そのページのヘッダの[テンプレート・パラメータ](/02.content/headers/#template) を設定することにより、上書きできます。
...
この例では、タイトルと **blog** タクソノミーの設定がされています。2つ目の `---` の後にあるコンテンツが、サイトにHTMLとして表示される実際のコンテンツです。これは[マークダウン](/02.content/markdown/) で書かれ、後ほどの章で詳しく解説予定です。今のところ、`#` が　**見出し1** 、`**` が **太字** そして`_` が **斜体** ということを知っておいてください。
...
この記号を置くときには、**前** と **後** に空白行があるようにしてください。たとえば、次のように：
...
> [!TIPS]
> `pages` オブジェクトは、[マークダウンコンテンツの中で直接Twigを動かす](../headers/#process) ときには利用できません。しかしながら、 `page` オブジェクトと、その継承された `find()` メソッドは、使用可能です。そのため、上記の代わりに、`{% for p in page.find('/projects').children %}` が利用可能です。**pages** が、Gravで利用できるPageすべてを指し、**page** は、単独で、しばしば現在のPageを指すことに注意してください。
...
ショートコードのコアのプラグインでは、そのページで必要なCSSやJSファイルを保存するのに使っています。しかし、この機能は、あなたが必要と思うあらゆるデータ構造でも保存可能です。
